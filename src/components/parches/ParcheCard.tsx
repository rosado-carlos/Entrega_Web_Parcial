...
const DEFAULT_COVER_IMAGE = "https://picsum.photos/800/400";
...
export default function ParcheCard({ parche }: Props) {
  ...
  return (
    <article className="card h-100 shadow-sm">
      <img
        src={parche.coverImageUrl || DEFAULT_COVER_IMAGE}
      ...
      <div className="card-body d-flex flex-column">
        ...
        <p className="small mb-2">
          <strong>Invite:</strong> {parche.inviteCode}
        </p>

        <div className="d-flex flex-wrap gap-2 mt-auto">
          <Link className="btn btn-primary" to={`/parches/${parche.id}`}>
            View parche
          </Link>

          <Link className="btn btn-outline-secondary" to={inviteLink}>
            Open invite link
          </Link>
        </div>
      </div>

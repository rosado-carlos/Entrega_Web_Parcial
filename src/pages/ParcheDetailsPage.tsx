...
export default function ParcheDetailsPage() {
  ...
  const currentMembership = activeParche.members.find((member) => member.userId === currentUser.id);
  ...
  const invitePath = `/parches/new?invite=${encodeURIComponent(activeParche.inviteCode)}`;

  async function handleRoleChange(targetUserId: number, role: ParcheRoleEnum) {
    setStatus("loading");
    setMessage("");
    ...
    <main className="container py-4">
      <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
        ...
        <div className="d-flex flex-wrap gap-2">
          <Link to={`/parches/${activeParche.id}/plans/new`} className="btn btn-primary">Create plan</Link>
          <Link to={invitePath} className="btn btn-outline-secondary">Open invite link</Link>
        </div>
      </div>

      <FeedbackAlert status={visibleStatus} message={visibleMessage} className="mb-3" />
        ...
        <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
          <div>
            ...
            <p className="small text-muted mb-0">
              Share the code <strong>{activeParche.inviteCode}</strong> or the invite link to let classmates join.
            </p>
          </div>
          <Link to={invitePath} className="btn btn-sm btn-outline-primary">Use invite link</Link>
        </div>
      </section>
          ...
          <table className="table table-sm align-middle mb-0">
            ...
            <tbody>
              {activeParche.members.map((member) => {
                ...
                const isPending = pendingUserId === member.userId && status === "loading";
                    ...
                    <td>
                      {canManageRoles && !isOwner ? (
                        <button
                          ...
                          onClick={() => void handleRoleChange(member.userId, nextRole)}
                          disabled={isPending}
                        >
                          {isPending
                            ? "Saving..."
                        ...
                        <span className="text-muted small">
                          {isOwner ? "Owner role is fixed" : "Only the owner can change roles"}
                        </span>
                      )}
                    </td>
                  </tr>
      ...
      <section>
        <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-2">
          ...
          <Link to="/rankings" className="btn btn-sm btn-outline-dark">View rankings</Link>
        </div>
